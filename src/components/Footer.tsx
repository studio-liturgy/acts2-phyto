...
export function Footer({ className }: { className?: string }) {
  return (
    <footer
      className={cn(
        `mt-20 hidden md:block ${isTest ? "bg-[var(--brand-orange-dark)]" : "bg-[var(--brand-blue)]"} text-[var(--brand-white)]`,
        className,
      )}
    >
      <div className="mx-auto grid max-w-6xl grid-cols-3 items-center gap-4 px-6 py-8">
        <nav className="flex items-center gap-6 text-xs">
          ...
          <a href="/donate" target="_blank" rel="noopener noreferrer" className={linkCls}>
            Donate
          </a>
        ...
        <div className="flex justify-center">
          <Link to="/">
            <img
              ...
              className={isTest ? "h-6 w-auto" : "h-12 w-auto"}
            />
          </Link>
        </div>
